        ...
        for size in self.sizes:
            ...
            with timeit(b"det", "numpy parallel", locals()) as context:
                if context.execute:
                    ...
                    pool.map(np.det, (x_np for i in range(iterations)))

            context = timeit(b"det", "ceygen parallel", locals())
            context.__enter__()
